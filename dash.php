...
function validateForms() {
    ...
    if (right == "" || isNaN(right) || right < 0) {
        alert("Please enter a valid marks for right answer");
        return false;
    }
    if (wrong == "" || isNaN(wrong) || wrong < 0) {
        alert("Please enter a valid minus marks for wrong answer");
        return false;
    }
    if (time == "" || isNaN(time) || time < 1) {
        alert("Please enter a valid time limit for the test in minutes");
        return false;
    }
    if (tag == "") {
        alert("Please enter a tag for searching");
        return false;
    }
    if (desc == "") {
        alert("Please write a description for the quiz");
        return false;
    }
    return true;
}
...
<div class="row">
<div class="col-lg-6">
<span class="logo">EduMetrica ||  Admin Panel</span></div>
<?php
...
<nav class="navbar navbar-default title1">
  ...
  <li <?php if(@$_GET['q']==6) echo'class="active"'; ?>><a href="dash.php?q=6">Report</a></li>
    ...
    <li <?php if(@$_GET['q']==3) echo'class="active"'; ?>><a href="dash.php?q=3">Feedback</a></li>
        ...
        <li class="dropdown <?php if(@$_GET['q']==7 || @$_GET['q']==8) echo'active"'; ?>">
          ...
          <ul class="dropdown-menu">
            <li><a href="dash.php?q=7">Visit Report</a></li>
            <li><a href="dash.php?q=8">Visitor view</a></li>
...
{
echo  '<div class="panel title"><div class="table-responsive">';
//echo '<iframe width="560" height="400" src="chart/donot.php" allowfullscreen align="middle"></iframe>';
echo '<p align="center"><iframe src="chart/donot.php" allowfullscreen width=600 height="400" frameBorder="0"> Report </iframe></p>';
...
echo  '<div class="panel"><div class="table-responsive"><table class="table table-striped title1">
<tr><td><b>S.N.</b></td><td><b>Name</b></td><td><b>Gender</b></td><td><b>course</b></td><td><b>Email</b></td><td><b>Remove</b></td><td></td></tr>';
$c=1;
while($row = mysqli_fetch_array($result)) {
	$name = $row['name'];
	$gender = $row['gender'];
...
<span class="title1" style="margin-left:40%;font-size:30px;"><b>Enter Quiz Details</b></span><br /><br />
 ...
 <form class="form-horizontal title1" name="form" action="update.php?q=addquiz" onSubmit="return validateForms()"  method="POST">
<fieldset>
...
$result = mysqli_query($con,"SELECT * FROM quiz ORDER BY date DESC") or die('Error');
echo  '<div class="panel"><div class="table-responsive"><table class="table table-striped title1">';
echo '<p align="center"><iframe src="visit/donot.php" allowfullscreen width=800 height="400" frameBorder="0"> Report </iframe></p>';
...
<?php if(@$_GET['q']==8) {
  ...
  $username = "conqrit2_root";
$password = "conqrit2_conqrity";
$hostname = "127.0.0.1"; 

//connection to the database
// Create connection

$conn = new mysqli($hostname, $username, $password, "conqrit2_sample");
// Check connection
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
} 

$sql = "SELECT * FROM visitor_location";
...
//
echo  '<div class="panel"><div class="table-responsive"><table class="table table-striped title1">
<tr><td><b>S.N.</b></td><td><b>IP Address</b></td><td><b>Date</b></td><td><b>Time</b></td><td><b>Lat</b></td><td><b>Long</b></td><td><b>ISP</b></td><td><b>Country</b></td><td></td></tr>';
$c=1;

// if ($result->num_rows > 0) {
//     // output data of each row
//     while($row = $result->fetch_assoc()) {
//         echo "id: " . $row["id"]. " - country: " . $row["country_name"]. " " . $row["ISP"]. "<br>";
//     }
// } else {
//     echo "0 results";
// }
while($row = mysqli_fetch_array($result)) {
  ...
  $long = $row['longitude'];
  $eid = $row['id'];
  echo '<tr><td>'.$c++.'</td><td>'.$ip.'</td><td>'.$date.'</td><td>'.$time.'</td><td>'.$lat.'</td><td>'.$long.'</td><td>'.$isp.'</td><td>'.$country.'&nbsp;</td></tr>';
}
$c=0;
echo '</table></div></div>';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare a SQL query to search for tags based on the keyword
    $sql = "SELECT * FROM quiz WHERE tag LIKE '%$searchKeyword%'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo "No tags found matching the search keyword.";
    }
}
}
?>
